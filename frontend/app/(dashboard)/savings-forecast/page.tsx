...
import { getForecast } from "@/lib/report-helpers";
...
const PRODUCT_LABELS: Record<Product, string> = {
  savings_account: "Savings (3.5%)",
  fd:              "Fixed Deposit (5%)",
  etf_sip:         "ETF / SIP (12%)",
};
const SC_COLORS = { current: "#94A3B8", optimised: "#10B981", aggressive: "#3B82F6" };
const RISK_COLORS: Record<string, string> = { none: "#10B981", low: "#3B82F6", medium: "#F59E0B", high: "#F43F5E" };
...
export default function SavingsForecastPage() {
  ...
  if (!report.savings_forecast) return <EmptyState message="Forecast unavailable. Upload a statement." />;

  const fc  = getForecast(report);
  const cur = fc.currency;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "12px" }}>
        {[
          ...
          <div key={m.label} className="card-premium" style={{ padding: "16px" }}>
            ...
            <div style={{ fontSize: "17px", fontWeight: "800", color: m.color }}>{m.val}</div>
          </div>
        ))}
      </div>
      ...
      <div className="card-premium" style={{ padding: "22px" }}>
          ...
          <div style={{ display: "flex", gap: "6px" }}>
            {(Object.keys(PRODUCT_LABELS) as Product[]).map(p => (
              <button key={p} onClick={() => setProduct(p)} style={{
                ...
                background: product === p ? "#0F172A" : "#F1F5F9",
                color:      product === p ? "white"   : "#64748B",
              }}>
                {p === "savings_account" ? "Savings" : p === "fd" ? "FD" : "ETF"}
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "16px", marginBottom: "12px", flexWrap: "wrap" }}>
          {(Object.entries(SC_COLORS) as [string, string][]).map(([s, color]) => (
            <div key={s} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              <div style={{ width: "10px", height: "10px", borderRadius: "50%", background: color }} />
              <span style={{ fontSize: "11px", color: "#64748B", textTransform: "capitalize" }}>{s}</span>
            </div>
          ))}
        </div>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={chartData}>
            <defs>
              {(Object.entries(SC_COLORS) as [string, string][]).map(([s, color], i) => (
            ...
            {fc.investment_products.map((p, i) => (
              <div key={i} style={{ padding: "14px", borderRadius: "12px", border: "1px solid #F1F5F9", background: "#FAFAFA" }}>
                ...
                <div style={{ display: "flex", gap: "6px", marginBottom: "6px" }}>
                  <span style={{ fontSize: "10px", padding: "2px 7px", borderRadius: "20px", fontWeight: "600", color: "white", background: RISK_COLORS[p.risk] ?? "#94A3B8" }}>
                    {p.risk} risk
                  </span>
