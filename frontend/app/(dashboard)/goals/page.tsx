...
export default function GoalsPage() {
  ...
  const gp = report.goals;
  if (!gp?.goals?.length) return <EmptyState message="No goals yet. Upload a statement to generate your goal plan." />;

  const activeGoals = goals ?? gp.goals;
  const cur = report.summary.currency;

  const handleProgress = async (id: string, pct: number) => {
    setGoals(prev => (prev ?? gp.goals).map((g: any) => g.id === id ? { ...g, progress_percent: pct } : g));
    setSaving(id);
    try {
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontSize: "20px", fontWeight: "800", color: "#10B981" }}>
            {formatCurrency(gp.monthly_free_cash || 0, cur)}
          </div>
        </div>
      </div>

      {gp.recommended_allocation && (
        <div className="card-premium" style={{ padding: "18px" }}>
          <h2 style={{ fontSize: "13px", fontWeight: "600", color: "#0F172A", marginBottom: "12px" }}>
            Recommended monthly allocation
          </h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "10px" }}>
            {Object.entries(gp.recommended_allocation).map(([k, v]: any) => (
              <div key={k} style={{ textAlign: "center", padding: "10px", borderRadius: "10px", background: "#F8FAFC" }}>
                ...
                <div style={{ fontSize: "14px", fontWeight: "700", color: "#0F172A" }}>{formatCurrency(v, cur)}</div>
              </div>
            ))}
          </div>
        </div>
