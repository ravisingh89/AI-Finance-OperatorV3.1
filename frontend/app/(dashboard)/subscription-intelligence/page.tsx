...
import { getSubIntel } from "@/lib/report-helpers";
...
export default function SubscriptionIntelligencePage() {
  ...
  const intel = getSubIntel(report);
  const cur   = intel.currency;

  if (!intel.subscriptions.length) return (
    <EmptyState message="No subscriptions detected. Upload a statement with at least one month of recurring charges." />
  );

  const subs = filter === "all"
    ? intel.subscriptions
    : intel.subscriptions.filter(s => s.recommendation === filter);
      ...
      <div style={{ display: "flex", gap: "6px" }}>
        {(["all", "cancel", "switch", "keep"] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{
            ...
            {f === "all"
              ? `All (${intel.subscriptions.length})`
              : `${f} (${intel.subscriptions.filter(s => s.recommendation === f).length})`}
          </button>
        ))}
      </div>
          ...
          return (
            <div key={sub.merchant} className="card-premium" style={{ padding: "18px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "12px" }}>
                ...
                <span style={{ fontSize: "10px", fontWeight: "700", padding: "3px 10px", borderRadius: "20px", color: "white", background: style.badge }}>
                  {style.label}
                </span>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "12px" }}>
                {[
                  { label: "Monthly", val: formatCurrency(sub.monthly_cost, cur) },
                  { label: "Annual",  val: formatCurrency(sub.annual_cost, cur) },
                ].map(m => (
                  <div key={m.label} style={{ padding: "8px", borderRadius: "8px", background: "#F8FAFC", textAlign: "center" }}>
                    <div style={{ fontSize: "9px", color: "#94A3B8" }}>{m.label}</div>
                    <div style={{ fontSize: "13px", fontWeight: "700", color: "#1E293B" }}>{m.val}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ marginBottom: "12px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "10px", marginBottom: "4px" }}>
                  ...
                  <span style={{
                    fontWeight: "700",
                    color: sub.usage_score >= 70 ? "#10B981" : sub.usage_score >= 45 ? "#F59E0B" : "#F43F5E",
                  }}>{sub.usage_score}/100</span>
                </div>
                <div style={{ height: "6px", background: "#F1F5F9", borderRadius: "3px", overflow: "hidden" }}>
                  <div style={{
                    height: "6px", borderRadius: "3px", width: `${sub.usage_score}%`,
              ...
              {sub.alternative && (
                <div style={{ marginTop: "8px", display: "flex", justifyContent: "space-between", fontSize: "11px", background: "rgba(16,185,129,0.06)", borderRadius: "8px", padding: "8px 12px", color: "#10B981" }}>
